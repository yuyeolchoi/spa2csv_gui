...
> **▶ Windows 사용자:** [**Releases**](../../releases) 페이지에 바로 실행 가능한
> `.exe`가 있습니다 — Python 설치 불필요. 내려받아 더블클릭만 하면 됩니다.
...
추가한 뒤 `Start`를 누릅니다. 각 파일은 변환이 끝나면 왼쪽(원본) 목록에서
사라지고 오른쪽(결과) 목록으로 이동하며, 오른쪽에는 완성된 CSV 파일명 또는
파일별 실패 사유가 표시됩니다. `Clear Results`로 오른쪽 목록을 비웁니다.
변환은 스레드 풀에서 동시에 처리하므로 대량 파일도 빠르게 변환됩니다.

`Load Folder`는 하위 폴더까지 훑으므로, series별 하위 폴더(각각 OMNIC split 결과)를
담은 상위 폴더를 지정하면 모든 스펙트럼을 한 번에 불러옵니다.

> 대량 변환에는 `Load Folder`를 권장합니다. `Load Files` 다이얼로그는 Windows
> 제약으로 한 번에 수백 개까지만 선택되지만, `Load Folder`는 그런 제한이 없습니다.
> 폴더 안에 이미 변환된 `.csv` 파일이 있어도 무시하므로, 폴더를 다시 로드해도
> 기존 결과를 다시 불러오지 않습니다.
